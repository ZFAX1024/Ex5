...
2.  **代码调试经验**：
    在 Jupyter Notebook 环境下，利用分步执行的特点，可以方便地查看每一层的输出形状。实验中遇到的主要问题是维度匹配问题（如全连接层输入计算、绘图时的维度转换），通过查阅文档和断点调试得以解决。

3.  **结果评估**：
    LeNet-5 模型参数量小、训练速度快，在 MNIST 数据集上表现优异。实验证明，相比于全连接神经网络，CNN 通过共享权值和局部连接，能更有效地从图像中提取空间特征。
